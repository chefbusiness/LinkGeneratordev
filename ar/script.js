// Mailto Link Generator - JavaScript Functions for Arabic
...
function generateMailtoLink(validateInput = false) {
    ...
    const linkText = document.getElementById('linkText').value.trim() || 'إرسال بريد إلكتروني';

    // Only validate when explicitly requested (button click)
    if (validateInput) {
        // Validate that at least 'to' field is filled
        if (!to) {
            alert('يرجى إدخال عنوان بريد إلكتروني واحد على الأقل في حقل "إلى"');
            return;
        }
...
function copyToClipboard(elementId) {
    ...
    navigator.clipboard.writeText(text).then(function() {
        ...
        alert('خطأ في النسخ إلى الحافظة');
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize main app
    initializeApp();
    console.log('Mailto Generator (Arabic) initialized successfully!');
});
